# ComfyAI Studio Local Toolkit

ComfyAI Studio Local Toolkit is a community-maintained, local-first derivative
workspace built on [ComfyUI](https://github.com/comfyanonymous/ComfyUI). It
keeps the upstream node-based generation engine and adds a Windows launcher,
local media utilities, optional watermark cleanup tools, and an optional
InstantID-assisted face replacement pipeline for authorized media editing.

This repository publishes source code only. Model weights, user inputs, outputs,
custom nodes, local databases, downloaded third-party projects, logs, and built
executables are deliberately excluded. Download models and optional dependencies
from their original sources and follow their licenses.

## Added Components

- `ai_studio_launcher.py`: local launcher and browser-based control panel.
- `watermark_tools/`: CLI and localhost API for image and video cleanup.
- `video_faceswap_pipeline.py`: optional frame-by-frame InstantID workflow.
- `setup_*.bat` and `start_*.bat`: Windows helpers with configurable Conda paths.
- `.github/workflows/public-source-check.yml`: CI guard against publishing local
  data, model weights, generated binaries, large artifacts, or obvious secrets.

## Quick Start

The current helper scripts are tested on Windows. They discover the repository
from the script location, so the project does not need to live at `D:\ComfyUI`.
The default Conda environment root remains `D:\conda_envs` for compatibility
with the original local workspace. Override it before running a helper when
needed:
...
Issues caused by the added launcher or local toolkit should be reported here.
Issues reproducible in upstream ComfyUI should be reported to the
[upstream issue tracker](https://github.com/comfyanonymous/ComfyUI/issues).

See [NOTICE.md](NOTICE.md) for attribution and optional dependency notes,
...
for the maintainer release checklist. Planned derivative-layer improvements are
tracked in [docs/ROADMAP.md](docs/ROADMAP.md).

---

## Upstream ComfyUI Documentation

The original ComfyUI README is preserved below for upstream engine usage.
